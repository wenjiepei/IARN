...
function data_loader:load_data_by_index_t7(opt)
  ...
  local item_category = torch.load(item_category_dir)
...
--      if ul<opt.min_len or il < opt.min_len then error('ul<opt.min_len or il<opt.min_len') end
  ...
  self.class_size = self.train_T:max()
  ...
  for i = 1, self.class_size do
    local n = self.train_T:eq(i)
    each_class_size[i] = torch.sum(n)
    print('grade ' .. i .. ':', torch.sum(n))
    each_class_weight[i] = self.train_T:nElement() / torch.sum(n) 
  end 
  ...
  if opt.if_negative == 1 then
    ...
    if file_exists(data_f) then
      ...
      self.train_pairs = dd.train_pairs
      self.train_T = dd.train_T
    else
      local map_ind = torch.ByteTensor(#self.user_X, #self.item_X):fill(0)
      for m = 1, self.train_pairs:size(1) do
        local u = self.train_pairs[m][1]+1
        local v = self.train_pairs[m][2]+1
        map_ind[u][v] = 1
      end
      ...
      local function if_valid(u, p)
        if map_ind[u+1][p+1] == 1 then
          return false
--        elseif torch.sum(remove_user_ind:eq(u+1)) > 0 or torch.sum(remove_item_ind:eq(p+1)) > 0 then
--          return false
        else
          return true
        end 
      ...
      while 1 do
        ...
        local item_i = self.valid_item_ind[item_randp[indu]]-1
        ...
        indu = indu + 1
        if indu > user_randp:size(1) then indu = 1 end
        indi = indi + 1
        if indi > item_randp:size(1) then indi = 1 end
      end
      self.train_pairs = torch.cat(self.train_pairs, neg_pair, 1)
      self.train_T = torch.cat(self.train_T, neg_T, 1)
      print('size of negative pairs: ', neg_pair:size())
      print('size of negative pairs: ', neg_T:size())

      local randpp = torch.randperm(self.train_T:size(1))
      self.train_pairs = self.train_pairs:index(1, randpp:long())
      self.train_T = self.train_T:index(1, randpp:long())
